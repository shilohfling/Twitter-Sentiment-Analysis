## This program is the twitteR project with minimal comments.
## This is the version to collect data - not for the R lesson.
...
myTwitter$getAll <- function(termA, termB, max = 1000) {
  a <- 0
  myTweetData <- list()                              
  for (x in termA) {         
    a <- a + 1
...
## Remove Username and ID from the Data Frame per IRB 
...
myTwitter$findNReplace <- function(df, emDict, dfcol = "ASCII", dictcol = "R.encoding", dictdesc = "Description") {
  n <- 0
